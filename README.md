...
## � 详细部署教程 | Detailed Deployment Guide

> 💡 **新手友好**：以下教程会带你从零开始，一步步完成网站部署。
...
网站文件只存在于你的**本地电脑**里，就像你家里的装修别人看不见一样。我们需要将其部署到**服务器**上，通过公网 IP 或域名，让全世界都能访问。

### 💡 Step 3: 核心概念

| 概念 | 解释 |
|------|------|
| **云服务器** | 在网上租一台 24 小时开机的电脑，用来托管你的网站 |
| **IP 地址** | 服务器在互联网上的"门牌号"（如 `192.168.1.1`） |
| **域名** | 给"门牌号"起的别名，好记且优雅（如 `lokua.xyz`） |

### ☁️ Step 4: 选择服务商

推荐的服务商包括阿里云、腾讯云和雨云。**对于新手，我们推荐使用雨云**，因为它操作更便捷。

🔗 [前往雨云官网注册](https://www.rainyun.com/NjkxODQ5_) （需完成实名认证）

### 🛒 Step 5: 购买服务器

🔗 [进入购买页面](https://app.rainyun.com/apps/rcs/buy)

**选择地区：**
- **中国香港**：免备案，适合快速上线。建议选择"极速3网"或大带宽
- **中国大陆**：速度最快，但域名**必须备案**

**选择配置：**
> ⚠️ **最低配置要求**：CPU 至少 **2核**，内存 至少 **2GB**
...
| 配置项 | 推荐选择 |
|--------|----------|
| 操作系统 | `Ubuntu Server 22.04 LTS (BBR 加速)` |
| 预装软件 | ✅ 勾选 `宝塔面板（最新版）` |

> BBR 加速可以显著提升网站访问速度，宝塔面板则能让你通过可视化界面轻松管理服务器。

### 🔎 Step 8: 查看服务器

🎉 恭喜购买成功！前往控制台查看你的服务器：

🔗 [雨云服务器管理面板](https://app.rainyun.com/apps/rcs/list)

### 🔑 Step 9: 获取面板信息

1. 在服务器列表中，点击对应服务器的 **"管理"** 按钮
2. 向下滑动找到 **"预装软件信息"** 区域，你会看到：

```
==================================================================
BT-Panel default info!
==================================================================
外网ipv4面板地址: https://8.138.109.76:11162/0f87d8c3
内网面板地址:     https://10.1.254.221:11162/0f87d8c3
username: 0qrgbqkp
password: b9fe1a34
```

> ⚠️ **极度重要**：请务必保护好你的**面板地址、用户名和密码**，切勿截图发给他人或公开分享！

### 🔐 Step 10: 面板登录

1. 复制**外网面板地址**并在浏览器中打开
2. 使用上一步获取的 `username` 和 `password` 登录

> 💡 请直接**复制粘贴**，避免手动输入出错（字母 l 和数字 1，字母 O 和数字 0 容易混淆）

### ⚡ Step 11: 安装 LNMP 环境

首次登录面板后，会弹出"推荐安装套件"窗口：

1. 左侧选择 **"LNMP (推荐)"**
2. 点击绿色的 **"一键安装"** 按钮
3. ⏳ 等待 5-30 分钟（取决于服务器配置）
...
**方式二：GitHub Release**
```
https://github.com/lulokua/WarmPrompt/releases
```

### 📤 Step 13: 上传与解压

1. **进入目录**：宝塔面板左侧 → **"文件"** → 根目录 `/` → 双击进入 `home` 文件夹
2. **上传文件**：点击顶部 **"上传"** 按钮，选择下载的源码压缩包
3. **解压文件**：在文件上右键 → 选择 **"解压"**

### 🗄️ Step 14: 创建数据库

1. 点击左侧 **"数据库"** → **"添加数据库"**
2. 填写**数据库名**和**用户名**（例如都填 `warmprompt`）
3. 密码保持默认

> ⚠️ **记住密码**：请务必记录下**数据库名、用户名和密码**，稍后配置时需要用到！

### ⚙️ Step 15: 配置环境文件

1. 在 **"文件"** 管理中，进入解压出来的文件夹
2. 找到 `.env` 文件并双击打开
3. 修改数据库配置：
...
### 🤖 Step 16: 配置 DeepSeek AI

1. **注册账号**：访问 [platform.deepseek.com](https://platform.deepseek.com/usage) 并充值（最低 1 元）
2. **创建 API Key**：
   - 点击左侧 **"API Keys"**
   - 点击 **"Create API Key"**
   - 输入名称后创建
   - ⚠️ **立即复制**生成的 Key（以 `sk-` 开头），关闭后不可再查看！

3. **填入配置**：回到 `.env` 文件，填入 API Key：
...
1. 宝塔面板左侧 → **"网站"** → **"Node 项目"** → **"添加 Node 项目"**
...
1. 回到 **"文件"** 管理，进入网站目录
2. 找到并进入 **`database`** 文件夹
3. 在 `.sql` 文件上右键 → 选择 **"导入数据库"**
4. 选择之前创建的数据库，确认导入

> ⚠️ **重要**：导入数据库是让网站正常运行的关键步骤！

### 🎉 Step 19: 完成部署！

恭喜你！所有配置都已完成，现在可以访问你的网站了！
